...
async function connectToWhatsApp() {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            // console.log(groups);
            let aa = groups.map(item => {
                let owner = item.owner.replace(/@s\.whatsapp\.net$/, '')
                return `Group id: ${item.id} \nGroup Name: ${item.subject} \nOwner: ${owner}`
            }).join('\n\n');
    ...
    sock.ev.on("messages.upsert", async ({ messages, type }) => {
        ...
        if (type === "notify") {
            if (!messages[0].key.fromMe) {
                ...
                const pesan = extractText(messages[0]);
                ...
                if (!messages[0].key.fromMe && pesanMasuk === "ping") {
                    ...
                    sock.sendMessage(noWa, {text: 'Saya Bot'}, { quoted: messages[0]})
                }
            }
        }
    });
...
function extractText(message) {
    if (message.message && message.message.extendedTextMessage) {
        ...
        if (extendedTextMessage && extendedTextMessage.text) {
          return extendedTextMessage.text;
        }
    }

    if (message.message && typeof message.message.conversation === "string") {
